def block_code(str):
    global C_block
    setStr = set(str)
    nDiffChars = len(setStr)
    nNeededBits = nDiffChars.bit_length()
    print ("setStr =",setStr)
    print ("nDiffChars =",nDiffChars)
    print ("nNeededBits =",nNeededBits)
    for index, char in enumerate(setStr):
        C_block.append( (char, bin(index)[2:].zfill(nNeededBits)) )
...
def main():
    ...
    block_code(str1)
    print ("C_block")
    [print(tupla) for tupla in C_block]
    print ("")
    C_block = []
...
setStr = {'O', 'N', 'j', 'z', 'u', 'ú', 'e', 'F', 'ü', 'í', 'c', 'T',
'S', 'ó', 'x', '\n', 'm', 'v', 'q', 'y', 'H', 'E', '.', 't', 'á', 'L',
',', 'r', 'p', 'i', 'n', 'Y', 'a', '¿', 'A', 'g', 'b', '-', 'U', 'P',
's', 'C', 'f', 'B', 'I', ' ', ';', 'G', '?', 'Q', 'o', 'é', 'R', ':',
'd', 'M', 'D', 'ñ', '¡', 'h', '!', 'l'}
nDiffChars = 62
nNeededBits = 6
